		...
		try {
			...
			for(int i = 0; json.getJSONArray("terms").getJSONObject(0).getJSONArray("sections").length() > i ; i++){
				if(json.getJSONArray("terms").getJSONObject(0).getJSONArray("sections").getJSONObject(i).getJSONArray("meetingPatterns").length() > 0){
					...
					for(int j = 0; json.getJSONArray("terms").getJSONObject(0).getJSONArray("sections").getJSONObject(i).getJSONArray("meetingPatterns").length() > j; j++){
						al.add(json.getJSONArray("terms").getJSONObject(0).getJSONArray("sections").getJSONObject(i).getJSONArray("meetingPatterns").get(j).getJSONArray("daysOfWeek").get(j));
					}
